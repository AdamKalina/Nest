...
folderTab::folderTab(QWidget *parent)
{
    ...
    refresh_selected_button = new QPushButton;
    refresh_selected_button->setText(tr("Refresh selected"));
    refresh_selected_button->setMinimumWidth(180);
    refresh_selected_button->setIcon(this->style()->standardIcon(QStyle::SP_BrowserReload));

    QLabel *helpLabel = new QLabel(tr("Double click on item for context menu"));
    helpLabel->setFont(f);

    QHBoxLayout *hlayoutStatic = new QHBoxLayout;
    hlayoutStatic->addWidget(refresh_selected_button, Qt::AlignLeft);
    ...
    connect(refresh_selected_button,SIGNAL(clicked()), this, SLOT(refresh_selected()));
}
...
void folderTab::refreshEntry(){
    QElapsedTimer timer;
    timer.start();
    mainwindow->checkDataOnHDD(currentItem->data(Qt::DisplayRole).toString(), false, recordingSystem); // refresh all folders as static when ran from here
    //qDebug() << "The slow operation took" << timer.elapsed() << "milliseconds";
    mainwindow->updatePatientTreeModel();
    dialog->close();
}

void folderTab::refresh_selected(){
    ...
    for(int i = 0; i<dfolder_path_list->selectedItems().count(); i++){
        mainwindow->checkDataOnHDD(dfolder_path_list->selectedItems().at(i)->text(), false, recordingSystem);
    }
    mainwindow->updatePatientTreeModel();
}
